...
void makeCandidate()
{
    for (int order = 0; order < gameCount; order++)
    {
...
void makeSudoku(){
    for(int order = 0 ; order < gameCount; order++){
        ...
        while(!pos.empty()){
            ...
            int pivot = -1; // ex)  -/5 일 때  -자리에 5보다 작은 수가 들어가야함 pivot =5 ,updown=false
